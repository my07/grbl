...
/* The cpu_map.h files serve as a central pin mapping selection file for
   different processor types or alternative pin layouts. This version of Grbl
   officially supports only the Arduino Mega328p. */
...
#define LIMIT_MASK                                                             \
  ((1 << X_LIMIT_BIT) | (1 << Y_LIMIT_BIT) |                                   \
   (1 << Z_LIMIT_BIT)) // All limit bits
#endif
...
// Z Limit pin and spindle PWM/enable pin swapped to access hardware PWM on
// Pin 11.
#ifdef VARIABLE_SPINDLE
#ifdef USE_SPINDLE_DIR_AS_ENABLE_PIN
// If enabled, spindle direction pin now used as spindle enable, while PWM
// remains on D11.
#define SPINDLE_ENABLE_BIT                                                     \
  5 // Uno Digital Pin 13 (NOTE: D13 can't be pulled-high input due to LED.)
#else
...
// Variable spindle configuration below. Do not change unless you know what you
// are doing. NOTE: Only used when variable spindle is enabled.
#define SPINDLE_PWM_MAX_VALUE                                                  \
  255 // Don't change. 328p fast PWM mode fixes top value as 255.
...
#define SPINDLE_TCCRA_INIT_MASK                                                \
  ((1 << WGM20) | (1 << WGM21)) // Configures fast PWM mode.
// #define SPINDLE_TCCRB_INIT_MASK   (1<<CS20)               // Disable
// prescaler -> 62.5kHz #define SPINDLE_TCCRB_INIT_MASK   (1<<CS21) // 1/8
// prescaler -> 7.8kHz (Used in v0.9) #define SPINDLE_TCCRB_INIT_MASK ((1<<CS21)
// | (1<<CS20)) // 1/32 prescaler -> 1.96kHz
#define SPINDLE_TCCRB_INIT_MASK                                                \
  (1 << CS22) // 1/64 prescaler -> 0.98kHz (J-tech laser)
...
// Dual axis feature requires an independent step pulse pin to operate. The
// independent direction pin is not absolutely necessary but facilitates easy
// direction inverting with a Grbl $$ setting. These pins replace the spindle
// direction and optional coolant mist pins.
...
// NOTE: Dual axis limit is shared with the z-axis limit pin by default. Pin
// used must be on the same port as other limit pins.
...
// NOTE: Coolant flood moved from A3 to A4. Coolant mist not supported with dual
// axis feature on Arduino Uno.
...
// NOTE: Spindle enable moved from D12 to A3 (old coolant flood enable pin).
// Spindle direction pin is removed.
#define SPINDLE_ENABLE_DDR DDRB
#define SPINDLE_ENABLE_PORT PORTB
#ifdef VARIABLE_SPINDLE
