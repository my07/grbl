/*
  serial.c - Low level functions for sending and recieving bytes via the serial
  port Part of Grbl
...
uint8_t serial_get_rx_buffer_available() {
  uint8_t rtail =
      serial_rx_buffer_tail; // Copy to limit multiple calls to volatile
...
// NOTE: Deprecated. Not used unless classic status reports are enabled in
// config.h.
...
uint8_t serial_get_tx_buffer_count() {
  uint8_t ttail =
      serial_tx_buffer_tail; // Copy to limit multiple calls to volatile
...
void serial_write(uint8_t data) {
  ...
  while (next_head == serial_tx_buffer_tail) {
    // TODO: Restructure st_prep_buffer() calls to be executed here during a
    // long print.
...
ISR(SERIAL_UDRE) {
  uint8_t tail = serial_tx_buffer_tail; // Temporary serial_tx_buffer_tail (to
                                        // optimize for volatile)
  ...
  // Turn off Data Register Empty Interrupt to stop tx-streaming if this
...
uint8_t serial_read() {
  uint8_t tail = serial_rx_buffer_tail; // Temporary serial_rx_buffer_tail (to
                                        // optimize for volatile)
...
ISR(SERIAL_RX) {
  ...
  // Pick off realtime command characters directly from the serial stream. These
  // characters are not passed into the main buffer, but these set system state
  // flag bits for realtime execution.
  switch (data) {
  case CMD_RESET:
    mc_reset();
  ...
  default:
    if (data > 0x7F) { // Real-time control characters are extended ACSII only.
      ...
      case CMD_JOG_CANCEL:
        if (sys.state &
            STATE_JOG) { // Block all other states from invoking motion cancel.
      ...
      // Throw away any unfound extended-ASCII character by not passing it to
      // the serial buffer.
